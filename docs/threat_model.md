# ChangeOps Threat Model

## Trusted Components

*   **ChangeOps Binary:** Must be compiled from untampered source code and run in a trusted local environment.
...
*   **Trusted Repository Configuration:** The `config/changeops-config.json` is assumed to be controlled by administrators and resistant to unauthorized tampering.
*   **Approval Key / Signing Authority:** The HMAC-SHA256 secret key located outside the repository (via `CHANGEOPS_APPROVAL_KEY_FILE`) is considered secure.
*   **HowlFrame Runtime and Policy Artifact:** The `changeops.hfbc` compiled policy file is assumed to reflect genuine administrator intent and cannot be tampered with by untrusted actors.
...
*   **Compromised Host / Root Access:** If an attacker has root on the host machine, they can extract the signing key, replace the ChangeOps binary, or directly bypass ChangeOps to interact with Git.
...
*   **Stolen HMAC Key:** If the key configured via `CHANGEOPS_APPROVAL_KEY_FILE` is compromised, an attacker can mint valid approvals for any decision.
